...
| `bun run ui:format` / `api:format` | Format with Prettier |
| `bun run ui:format.check` / `api:format.check` | Check formatting (used in CI) |
| `bun run ui:check` / `api:check` | Typecheck with `tsc` |
...
| `ALLOWED_ORIGINS` | Comma-separated CORS origins allowed to call the API (default: `http://localhost:3030,http://127.0.0.1:3030`) |
